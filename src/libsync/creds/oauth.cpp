...
void OAuth::fetchWellKnown()
{
    ...
    if (!urls.first.isNull()) {
        ...
        QNetworkRequest req;
        req.setAttribute(HttpCredentials::DontAddCredentialsAttribute, true);
        req.setUrl(Utility::concatUrlPath(
            _serverUrl,
            QStringLiteral("/.well-known/webfinger"),
        ...
        connect(reply, &QNetworkReply::finished, this, [reply, this] {
            ...
            auto const issuerUrl = QUrl((*link).value(QStringLiteral("href")).toString());
            auto const properties = (*link).value(QStringLiteral("properties")).toObject();
            const auto clientIdProperty = QStringLiteral("http://openid.net/specs/connect/1.0/client_id");
            if (const auto clientId = properties.value(clientIdProperty).toString(); !clientId.isNull()) {
                _clientId = clientId;
            }
            const auto scopeProperty = QStringLiteral("http://openid.net/specs/connect/1.0/scope");
            if (const auto scope = properties.value(scopeProperty).toString(); !scope.isNull()) {
            ...
            QNetworkRequest req;
            req.setAttribute(HttpCredentials::DontAddCredentialsAttribute, true);
            req.setUrl(Utility::concatUrlPath(issuerUrl, wellKnownPathC));
            req.setTransferTimeout(defaultTimeoutMs());
